...
2. **升级阶段**：bootloader 复位后以其编译期写死的波特率（115200）通信

USB-TTL 芯片（CH340/CP210x 等）可在运行时随时切换波特率，这两段通信无缝衔接。
蓝牙模块的波特率在配对时即固定，**中途无法切换**，因此走蓝牙链路时必须人工把
两端统一成同一个波特率（要么改四个生成器的 `UART_Init` 把 App 也设成 115200，
要么改 `config.h` 把 bootloader 设成 230400 后重新烧底）。这一限制在
     ...
     └─ 关中断 -> 写 DfuFlag=0x12abcd34 -> IAP_CONTR=0x20 软复位
        └─ 芯片复位 -> bootloader 启动 (dfu_check)
           └─ 读到 DfuFlag == DFU_TAG，停留在下载模式
              ├─ CONNECT  -> 返回版本号 0x0100   (115200 baud)
...
下载过程有一次波特率切换：230400 发触发字 -> 115200 与 bootloader 通信
（见 §2.2 对蓝牙链路的限制说明）。
...
（当前 115200，由 `config.h` 的 `FOSC` 推导）。蓝牙链路还需统一 App 的
230400 与 bootloader 的 115200（见 §2.2），两者都需要人工决定后改代码重新烧底。
