...
| 命令码 | 含义 | 参数 |
| --- | --- | --- |
| `Init_Order` 0xAA | 初始化：设定 PWM 组频率 | `50`=摩擦轮/舵机/抖动电机，`10000`=平滑电机；PWMA/PWMB 组内四路必须同频 |
...
- 所有端口都可作舵机使用，**初始化频率按 PWM 组统一设置**：`50`=舵机/摩擦轮/抖动电机，
  `10000`=平滑电机
- PWMA 包含 P60/P62/P64/P66，PWMB 包含 P74/P75/P76/P77；同组四路必须同频。
- 步兵 PWMA 固定 50Hz、PWMB 固定 10000Hz；平滑电机占用 PWMA 时允许生成但会出现 Warn，实机可能一卡一卡。
- 工程 PWMA/PWMB 均可自由选择 50Hz 或 10000Hz，角色与组频率不匹配时提示 Warn。
- **同一端口在同一时刻不能同时作电机和舵机**
