...
利用手机的陀螺仪/加速度计和 ARCore 视觉惯性里程计，给 pie-block 机械臂仿真发送目标位姿。
...
1. 安装 [Android Studio](https://developer.android.com/studio)（Hedgehog 2023.1.1 或更高）
2. 打开 `android_app/` 目录
3. 等待 Gradle 同步完成
4. 连接 Android 手机（minSdk 24 = Android 7.0，需支持 ARCore）
5. 点击 Run

> **注意**: ARCore 需要手机支持 Google Play Services for AR。首次运行 APP 会提示安装。
...
1. 确保手机和 PC 连同一 WiFi
2. 打开 APP
3. 输入 PC 的 IP 地址（或点「扫码」扫描 Godot 侧显示的地址）
4. 点「连接」
5. 连接后移动手机：
