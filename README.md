...
新版继续使用 `.pieproj` 扩展名，但只接受 `format_version: 14`。格式 13 和旧 Godot 格式不会自动转换，需在新版中重新创建配置。新项目不预填任何必填配置；调试工程预置十路停用引脚和 3 秒安全时长。音乐工程默认 480 PPQ、120 BPM 和 4/4 拍，可重新导入原始 MIDI。详细结构见 [项目文件格式](docs/Flutter项目文件格式.md)。
