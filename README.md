...
暂不提供调试/音乐项目、云端编译、串口/蓝牙烧录、AI 编辑、3D 仿真、Android、Web、CLI 或 MCP。
...
新版继续使用 `.pieproj` 扩展名，但只接受 `format_version: 14`。格式 13 和旧 Godot 格式不会自动转换，需在新版中重新创建配置。新项目不预填任何必填配置；步兵摩擦轮仅在选择“无刷电调”后占用 P64/P66。详细结构见 [项目文件格式](docs/Flutter项目文件格式.md)。
