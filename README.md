...
`main.c`，再调用外置 Keil C251 或云端编译服务编译成 hex。

技术栈：Godot 4.7（GDScript）+ 外置 Keil C251 / 云端编译服务 + ttyd/WRY（内嵌 AI 终端）。
...
- 本地编译使用用户指定的外置 Keil C251；也可选择云端编译，本机无需安装 Keil
- 图形化配置页与 AI 编辑页共用 `BuildController`，统一项目部署、写盘、后台编译、成功判据与日志
- 外置 Keil 的 `TOOLS.INI` 由 Keil 安装程序维护，应用只读取并按需更新 C251 许可证
