...
func _setup_build_mode_selector() -> void:
	...
	if opt == null:
		...
		return
	_build_mode = opt
	_build_mode.select(1 if _tc.get_selected_compiler() == TC.COMPILER_KEIL else 0)
	...
	if not _tc.is_sdcc_supported_platform():
...
func _on_compiler_selected(_index: int) -> void:
	if not _tc.set_selected_compiler(_selected_compiler()):
...
func _do_build() -> void:
	...
	_build_controller.start(_project_dst, code, AppState.project_kind, _selected_compiler())
...
func _do_hex_export() -> void:
	...
	if not _build_controller.start(_project_dst, code2,
			AppState.project_kind, _selected_compiler()):
		_hex_export_pending = false
