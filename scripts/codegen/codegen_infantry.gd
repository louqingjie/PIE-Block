...
## 不允许任何一侧另抄一份公式，避免基类更新后仿真与固件数值不一致。
##
## 返回键：
...
func generate(cfg: Dictionary) -> String:
	...
	if friction_enabled:
		code += "void CalculateBoosterControl()\n{\n"
		code += "    // 非阻塞状态机：开关和增减速只修改目标值；运行区间从 500 duty 起步，斜坡每轮变化 1。\n"
		code += "    // 摩擦轮开关由 %s 上升沿触发；增减速键也只响应上升沿。\n" % cfg.get("booster_key", "A")
		code += "    uint8_t frictionStartedThisCycle = 0;\n"
		...
		code += "            frictionStartedThisCycle = 1;\n"
		code += "        }\n"
		...
		code += "    {\n"
		code += "        if (!frictionStartedThisCycle)\n"
		code += "        {\n"
		code += "            if (targetDutyOfBooster > dutyOfBooster)\n"
		code += "                dutyOfBooster += FRICTION_STEP_DUTY;\n"
		code += "            else if (statusOfBooster == 0 && dutyOfBooster <= FRICTION_START_DUTY)\n"
