...
void nrf_writebuf(uint8_t reg, uint8_t *pBuf, uint16_t len) __reentrant;
void nrf_readbuf(uint8_t reg, uint8_t *pBuf, uint16_t len) __reentrant;
...
uint8_t NRF24L01_Init(void)
{
  ...
  nrf_writereg(NRF_WRITE_REG + RF_CH, Channal); // ����RFͨ��ΪCHANAL
...
void P2_INT_ISR_Handler(void) __interrupt (P2INT_VECTOR)
{
