 ...
 * __reentrant），若在初始化期间抢先执行，会破坏 nrf_link_check 的 SPI 校验，
 ...
 *   - 进入某步前：LED 显示该步编码（3 __bit 二进制，P35=bit0 P36=bit1 P37=bit2）
 ...
 * 注意：参数不能叫 __code —— __code 是 C251 保留字（存储类型限定符） */
...
void main(void)
{
     ...
     * 彻底避免 ISR 里 SPI/__reentrant 死锁（实测遥控器开着会导致初始化卡死） */
    P2INTE &= ~GPIO_Pin_6;
    EA = 1;
    StepDone(4);
