 ...
 *   - 默认波特率 115200；不要加 \r\n 结尾（部分固件把换行算进参数拒收） */
static unsigned long __code BAUD_TABLE[] = {9600, 115200, 19200, 38400, 57600, 230400, 1200, 2400, 4800};
#define BAUD_COUNT 9
static char __code CMD_VER[] = "AT+VER";              /* 探测用：读版本（只读安全） */
static char __code CMD_ISCEN1[] = "AT+ISCEN1";        /* 打开密码认证（默认关闭，必须先开） */
static char __code CMD_SET_PASS[] = "AT+PASS0000";    /* 4 位密码 0000（V2.1 手册：只能 4 位） */
static char __code CMD_SET_PASS6[] = "AT+PASS000000"; /* 6 位密码回退（V3.x 固件用） */
static char __code CMD_RST[] = "AT+RST";              /* JDY-08 原生复位指令 */
static char __code CMD_RESET[] = "AT+RESET";          /* HM-10 clone 固件的复位指令 */

/* ==================== UART1 收发工具 ==================== */

/* 发送一串 __code 段字符串 */
static void Uart1SendStr(char __code *s)
{
    while (*s)
